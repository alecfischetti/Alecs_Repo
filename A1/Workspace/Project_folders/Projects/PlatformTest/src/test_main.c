...
//
//*****************************************************************************
am_hal_uart_config_t g_sUartConfig = { .ui32BaudRate = 115200, .ui32DataBits =
		AM_HAL_UART_DATA_BITS_8, .bTwoStopBits = false, .ui32Parity =
		AM_HAL_UART_PARITY_NONE, .ui32FlowCtrl = AM_HAL_UART_FLOW_CTRL_NONE, };
...
void print_test_menu(void) {
	...
	test_printf(
			"1.\tGpio Get <1 Gpio# >, Returns current Value of this Gpio  \n");
	...
	test_printf(
			"4.\tGpio Toggle <4 Gpio#> Change the state of specified gpio\n");
	test_printf(
			"5.\tGpio Replace <5 Gpio# (0=Low 1=High)> Set this Gpio pin high\n");
	test_printf(
			"6.\tGpio Set Direction <6 Gpio# In (0) or Out(1)> Sets Direction of this Gpio \n");
	...
	test_printf("13.\tTest ADX1362 [poll accellerometer]\n");
...
void readCmd(cmd_test_enum *cmd1, uint32_t *cmd2, uint32_t *cmd3) {
	...
	*cmd2 = command;
	...
	if (*cmd1 == CMD_TEST_GPIO_REPLACE || *cmd1 == CMD_TEST_GPIO_SET_DIR
			|| *cmd1 == CMD_TEST_I2C) {
		index = index + len;
		command = convertStrToDec(&rxBuffer[index], &len);
		am_util_stdio_printf(" %d \n", command);
...
void processCmd(cmd_test_enum TestCommand1, uint32_t TestCommand2,
		uint32_t TestCommand3) {
	...
	case CMD_TEST_GPIO_TOGGLE:
		am_util_stdio_printf("Change State Of GPIO %d to !GPIO %d",
	...
	case CMD_TEST_GPIO_REPLACE:
		am_util_stdio_printf("Set State Of GPIO %d to %d", TestCommand2,
				TestCommand3);
	...
	case CMD_TEST_NORMAL_SLEEP:
		test_printf("Runing Normal Sleep Test \n \n");
		uart_transmit_delay();
...
int main(void) {
	...
	am_util_stdio_printf_init(
			(am_util_stdio_print_char_t) am_hal_uart_string_transmit_polled);
