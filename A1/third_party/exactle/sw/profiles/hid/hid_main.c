...
uint8_t HidGetProtocolMode()
{
  ...
  if (hidCb.pConfig->deviceType == HID_DEVICE_TYPE_KEYBOARD)
  {
    AttsGetAttr(HIDKB_PROTOCOL_MODE_HDL, &len, &pValue);
  }
  else
  {
    AttsGetAttr(HIDM_PROTOCOL_MODE_HDL, &len, &pValue);
  }
...
void HidSetProtocolMode(uint8_t protocolMode)
{
  ...
  if (hidCb.pConfig->deviceType == HID_DEVICE_TYPE_KEYBOARD)
  {
    AttsSetAttr(HIDKB_PROTOCOL_MODE_HDL, 1, &protocolMode);
  }
  else
  {
    AttsSetAttr(HIDM_PROTOCOL_MODE_HDL, 1, &protocolMode);
  }
}
                          ...
                          attsAttr_t *pAttr)
{
  ...
  case HIDKB_PROTOCOL_MODE_HDL:
  case HIDM_PROTOCOL_MODE_HDL:
  ...
  case HIDKB_KEYBOARD_BOOT_OUT_HDL:
...
void HidInit(const hidConfig_t *pConfig)
{
  ...
  /* Store the report map */
  AttsSetAttr(HID_REPORT_MAP_HDL, pConfig->reportMapSize, pConfig->pReportMap);

  /* Register the atttribute read/write functions */
  switch(pConfig->deviceType)
  {
  case HID_DEVICE_TYPE_KEYBOARD:
    SvcHidKeyboardRegister(HidAttsWriteCback, NULL);
    break;
  case HID_DEVICE_TYPE_MOUSE:
    SvcHidMouseRegister(HidAttsWriteCback, NULL);
    break;
  case HID_DEVICE_TYPE_GENERIC:
    SvcHidGenericRegister(HidAttsWriteCback, NULL);
    break;
  }
}