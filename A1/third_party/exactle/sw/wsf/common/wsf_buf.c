...
void WsfBufGetPoolStats(WsfBufPoolStat_t *pStat, uint8_t numPool)
{
  ...
  pPool = (wsfBufPool_t *) wsfBufMem;
  numPool = WSF_MIN(numPool, wsfBufNumPools);

  for (i = 0; i < numPool; i++)
  {
    pStat[i].bufSize  = (uint8_t)pPool[i].desc.len;
    pStat[i].numBuf   = pPool[i].desc.num;
    pStat[i].numAlloc = pPool[i].numAlloc;
    pStat[i].maxAlloc = pPool[i].maxAlloc;
  }
