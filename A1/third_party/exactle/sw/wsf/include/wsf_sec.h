...
/*! AES anc CMAC Security callback are the same as wsfSecMsg_t */
...
typedef struct
{
  uint8_t pubKey_x[WSF_ECC_KEY_LEN];
  uint8_t pubKey_y[WSF_ECC_KEY_LEN];
  uint8_t privKey[WSF_ECC_KEY_LEN];
} wsfSecEccKey_t;

typedef struct
{
  uint8_t secret[WSF_ECC_KEY_LEN];
...
typedef struct
{
  ...
  union
  {
    wsfSecEccSharedSec_t sharedSecret;
    wsfSecEccKey_t key;
  } data;
} wsfSecEccMsg_t;
 ...
 */
/*************************************************************************************************/
uint8_t WsfSecCmac(const uint8_t *pKey, uint8_t *pPlaintext, uint8_t textLen, wsfHandlerId_t handlerId, 
                   uint16_t param, uint8_t event);
